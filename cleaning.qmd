...
breitband_1km_arrow %>%
  ...
  group_by(x_mp, y_mp) %>%
  mutate(got_fiber = has_fiber - lag(has_fiber, n = 1)) %>%
  ungroup() %>%
  filter(got_fiber == -1) %>%
  distinct(x_mp, y_mp) %>%
  to_arrow() %>%
...
```{r}
data_bbsr <- readxl::read_xlsx("./Daten/Raumgliederung_BBSR/raumgliederungen-referenzen-2022.xlsx", 
...
Ich lade die Geodaten der Gemeinden. Außerdem lade ich die 1km Grid-Zellen und füge die sf Geometrie hinzu.

```{r}
gemeinden <- st_read("./Daten/Geodaten/gemeinden.gpkg", quiet = TRUE, as_tibble = TRUE)
gemeinden

kauf_distinct_cells <- open_dataset("./Daten/Immo/Kauf/") %>%
  filter(ejahr >= 2018) %>%
  distinct(x_mp, y_mp) %>%
  collect()

miete_distinct_cells <- open_dataset("./Daten/Immo/Miete/") %>%
  filter(ejahr >= 2018) %>%
  distinct(x_mp, y_mp) %>%
  collect()

all_distinct_cells <- bind_rows(kauf_distinct_cells, miete_distinct_cells) %>%
  distinct(x_mp, y_mp) %>%
...
Manche Zellen liegen in keiner Gemeinde. Nach visueller Inspektion liegen diese Zellen mit ihrem Mittelpunkt nicht in Deutschland. Es handlet sich auch nicht um viele Zellen. Sie werden daher verworfen.

```{r}
cells_gemeinden_bbsr <- st_join(all_distinct_cells, gemeinden) %>%
  st_drop_geometry() %>%
  left_join(data_bbsr, by = join_by(nsi_code == gem_kennziffer)) %>%
  filter(complete.cases(.)) %>%
  select(x_mp, y_mp, ags = nsi_code, gem_name, nuts_code, dgurb_kennziffer:last_col())
