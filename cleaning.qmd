...
kreise_einkommen <- readxl::read_xlsx("./Daten/Sonstige/vgrdl_r2b3_bs2023.xlsx", sheet = "2.4", skip = 4) %>%
  janitor::clean_names() %>%
  drop_na(lfd_nr) %>%
  filter(nuts_3 == 3) %>%
...
cells_kreise_einkommen %>%
  write_parquet("./Daten/Sonstige/Raster_1km_kreise_einkommen.parquet")
```
