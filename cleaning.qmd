...
arrow_dates <- breitband_arrow %>% 
  distinct(date) %>%
  arrange(date) %>%
  collect() %>%
  pull(date)

arrow_variables <- breitband_arrow %>%
  filter(date == 202312) %>%
  slice_sample(n = 1) %>%
  collect() %>%
...
for(d in arrow_dates) {
  ...
  for(v in arrow_variables) {
    ...
    breitband_arrow %>%
      ...
      drop_na(anzahl_haushalte, !!sym(v)) %>%
...
```{r}
breitband_1km_arrow <- open_dataset("./Daten/Breitbandatlas/Raster_1km_long/")
