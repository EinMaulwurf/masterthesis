...
```{r}
st_layers("../Daten/Geodaten/DE_VG250.gpkg")

deutschland <- st_read("../Daten/Geodaten/DE_VG250.gpkg", layer = "v_vg250_sta", as_tibble = TRUE) %>%
  st_transform(3035) %>%
  janitor::clean_names() %>%
  filter(id == 1) %>%
  select(name = geografischer_name_gen, einwohner = einwohnerzahl_ewz, flaeche = katasterflache_kfl)

bundesländer <- st_read("../Daten/Geodaten/DE_VG250.gpkg", layer = "v_vg250_lan", as_tibble = TRUE) %>%
  st_transform(3035) %>%
  janitor::clean_names() %>%
...
einkommen <- read_parquet("../Daten/Geodaten/Raster_1km_kreise_einkommen.parquet") %>%
...
gemeinden_bbsr <- read_parquet("../Daten/Raumgliederung_BBSR/gemeinden_bbsr.parquet")
...
tidy(dat_att) %>%
  ...
  scale_color_manual(values = c("#7B3294", "#E67E22"))+
