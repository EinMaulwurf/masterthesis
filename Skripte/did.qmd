...
tidy(mod_miete_agg_dyn) %>%
  ...
  theme_bw()+
  theme(legend.position = "none")+
  labs(x = "Halbjahr: Rel. zum ersten Treatment",
       y = "ATT: Miete (€/m²)")
...
tidy(mod_miete_agg_group) %>%
  ...
  theme_bw()+
  theme(legend.position = "none")+
  labs(y = "Halbjahr: Rel. zum ersten Treatment",
       x = "ATT: Miete (€/m²)")
...
ggsave("../Output/plots/did/plot_did_räumliche_miete.pdf", width = 3, height = 3, device = cairo_pdf)
```
...
models_räumlich_kauf_agg_dyn_tidy %>%
  ...
  theme_bw()+
  labs(x = "ATT(g, g+2): Kaufpreis (1000€)",
       y = "Modell")

ggsave("../Output/plots/did/plot_did_räumlich_kauf.pdf", width = 3, height = 3, device = cairo_pdf)
```
