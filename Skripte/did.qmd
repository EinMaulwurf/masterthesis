...
# Save results
write_parquet(models_subset_miete_agg_dyn_tidy, "../Daten/Sonstige/models_subset_miete_agg_dyn_tidy.parquet")
#models_subset_miete_agg_dyn_tidy <- read_parquet("../Daten/Sonstige/models_subset_miete_agg_dyn_tidy.parquet")
```

```{r}
models_subset_miete_agg_dyn_tidy %>%
...
model_specs_subset_kauf <- list(
  ...
  model3 = list(
    ...
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter + anzahl_haushalte + eigentuemerquote
  ),
  model4 = list(
...
models_subset_kauf <- model_specs_subset_kauf %>%
  map(~ modifyList(base_params_subset_kauf, .x)) %>%
  map(~do.call(att_gt, .x), .progress = TRUE)

# Calculate dynamig aggregation
models_subset_kauf_agg_dyn_tidy <- models_subset_kauf %>%
  purrr::map(
    ~did::aggte(.x, type = "dynamic"),
    .progress = TRUE
  ) %>%
...
# Save results
write_parquet(models_subset_kauf_agg_dyn_tidy, "../Daten/Sonstige/models_subset_kauf_agg_dyn_tidy.parquet")
#models_subset_kauf_agg_dyn_tidy <- read_parquet("../Daten/Sonstige/models_subset_kauf_agg_dyn_tidy.parquet")
```

```{r}
models_subset_kauf_agg_dyn_tidy %>%
  mutate(number_clean = str_extract(model, "\\d+$") %>% as.numeric(),
         name_clean = paste0("(", as.roman(number_clean), ")") %>% fct_reorder(number_clean, .desc = TRUE)) %>%
  filter(event.time == 2) %>%
  select(model, name_clean, estimate, conf.low, conf.high) %>%
