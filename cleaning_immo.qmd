...
data_kauf_imp <- data_kauf %>%
  mice::mice(m = 1, maxit = 1, method = "pmm", pred = quickpred(., exclude = c("x_mp", "y_mp")), seed = 1) %>%
  mice::complete() %>%
  as_tibble()
...
data_kauf_complete <- data_kauf_imp %>%
  ...
  inner_join(cells_gemeinden_bbsr %>% select(x_mp, y_mp, ends_with("_name")), by = join_by(x_mp, y_mp)) %>%
  inner_join(cells_kreise_einkommen, by = join_by(x_mp, y_mp)) %>%
...
data_miete_complete <- data_miete_imp %>%
  ...
  left_join(cells_kreise_einkommen, by = join_by(x_mp, y_mp)) %>%
