...
```{r}
gemeinden_bbsr <- readxl::read_xlsx("./data/Raumgliederung_BBSR/raumgliederungen-referenzen-2022.xlsx",
...
gemeinden_bbsr %>%
  write_parquet("./data/Raumgliederung_BBSR/gemeinden_bbsr.parquet")
```

```{r}
all_distinct_cells <- st_read("./data/LAEA_Grid/DE_Grid_ETRS89-LAEA_1km.gpkg",
...
cells_gemeinden_bbsr %>%
  write_parquet("./data/Raumgliederung_BBSR/Raster_1km_gemeinden_bbsr.parquet")
```
...
kreise_einkommen <- readxl::read_xlsx("./data/Geodaten//vgrdl_r2b3_bs2023.xlsx", sheet = "2.4", skip = 4) %>%
  janitor::clean_names() %>%
  drop_na(lfd_nr) %>%
  filter(nuts_3 == 3) %>%
...
cells_kreise_einkommen %>%
  write_parquet("./data/Geodaten/Raster_1km_kreise_einkommen.parquet")
```
...
plz <- st_read("./data/Geodaten/PLZ_Gebiete_9143106783908117499.gpkg",
                    as_tibble = TRUE, quiet = TRUE) %>%
  st_transform(3035) %>%
  janitor::clean_names() %>%
  select(plz)
...
cells_plz %>%
  write_parquet("./data/Geodaten/Raster_1km_plz.parquet")
```
...
zensus_gebäudetyp <- readxl::read_xlsx("./data/Zensus/Regionaltabelle_Gebaeude_Wohnungen.xlsx", 
...
zensus_gebäudetyp %>%
  write_parquet("./data/Zensus/zensus_gebäudetypen.parquet")
...
cells_gemeinden_gebäudetypen %>%
  write_parquet("./data/Geodaten/Raster_1km_gemeinden_gebäudetypen.parquet")
```
