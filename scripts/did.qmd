...
data_miete_complete <- read_parquet("./data/processed/RWI_GEO_RED/data_miete_complete.parquet")
data_kauf_complete <- read_parquet("./data/processed/RWI_GEO_RED/data_kauf_complete.parquet")
```
...
model_specs_subset_miete <- list(
  ...
  model2 = list(
    data = data_miete_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Großstadt"),
  ...
  model3 = list(
    data = data_miete_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Mittelstadt"),
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter + anzahl_haushalte + einkommen + einkommen_delta + eigentuemerquote
  ),
  model4 = list(
    data = data_miete_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Größere Kleinstadt"),
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter + anzahl_haushalte + einkommen + einkommen_delta + eigentuemerquote
  ),
  model5 = list(
    data = data_miete_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Kleine Kleinstadt"),
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter + anzahl_haushalte + einkommen + einkommen_delta + eigentuemerquote
  ),
  model6 = list(
    data = data_miete_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Landgemeinde"),
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter + anzahl_haushalte + einkommen + einkommen_delta + eigentuemerquote
  ),
  model7 = list(
    data = data_miete_complete %>% filter(n_cell_date > 0) %>% 
...
# Save results
write_parquet(models_räumlich_miete_agg_dyn_tidy, "./data/processed/Sonstige/models_räumlich_miete_agg_dyn_tidy.parquet")
#models_räumlich_miete_agg_dyn_tidy <- read_parquet("./data/processed/Sonstige/models_räumlich_miete_agg_dyn_tidy.parquet")
```
...
model_specs_subset_kauf <- list(
  ...
  model2 = list(
    data = data_kauf_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Großstadt"),
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter #+ anzahl_haushalte + einkommen + einkommen_delta + eigentuemerquote # Probleme mit Singularität
  ),
  model3 = list(
    data = data_kauf_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Mittelstadt"),
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter + anzahl_haushalte + einkommen + einkommen_delta + eigentuemerquote
  ),
  model4 = list(
    data = data_kauf_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Größere Kleinstadt"),
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter + anzahl_haushalte + einkommen + einkommen_delta + eigentuemerquote
  ),
  model5 = list(
    data = data_kauf_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Kleine Kleinstadt"),
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter + anzahl_haushalte + einkommen + einkommen_delta + eigentuemerquote
  ),
  model6 = list(
    data = data_kauf_complete %>% filter(n_cell_date > 0) %>% filter(gemeindetyp_name == "Landgemeinde"),
    xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + durchschnittsalter + anzahl_haushalte + einkommen + einkommen_delta + eigentuemerquote
  ),
  model7 = list(
    data = data_kauf_complete %>% filter(n_cell_date > 0) %>% 
...
# Save results
write_parquet(models_räumlich_kauf_agg_dyn_tidy, "./data/processed/Sonstige/models_räumlich_kauf_agg_dyn_tidy.parquet")
#models_räumlich_kauf_agg_dyn_tidy <- read_parquet("./data/processed/Sonstige/models_räumlich_kauf_agg_dyn_tidy.parquet")
```
...
# Save results
write_parquet(models_miete_agg_dyn_tidy, "./data/processed/Sonstige/models_miete_agg_dyn_tidy.parquet")
#models_miete_agg_dyn_tidy <- read_parquet("./data/processed/Sonstige/models_miete_agg_dyn_tidy.parquet")
```
...
# Save results
write_parquet(models_kauf_agg_dyn_tidy, "./data/processed/Sonstige/models_kauf_agg_dyn_tidy.parquet")
#models_kauf_agg_dyn_tidy <- read_parquet("./data/processed/Sonstige/models_kauf_agg_dyn_tidy.parquet")
```
