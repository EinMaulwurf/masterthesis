...
```{r}
import_hk <- read_parquet("./data/RWI_GEO_RED/Daten_Kauf/HK_SUF_ohneText.parquet",
...
import_wk <- read_parquet("./data/RWI_GEO_RED/Daten_Kauf/WK_SUF_ohneText.parquet",
                          col_select = all_of(col_kauf)) %>%
  haven::zap_labels() %>%
  haven::zap_formats() %>%
  haven::zap_label() %>%
...
data_kauf %>%
  write_parquet("./data/RWI_GEO_RED/Daten_Kauf/data_kauf.parquet")
```
...
```{r}
import_hm <- read_parquet("./data/RWI_GEO_RED/Daten_Miete/HM_SUF_ohneText.parquet",
...
import_wm <- read_parquet("./data/RWI_GEO_RED/Daten_Miete/WM_SUF_ohneText.parquet",
                          col_select = all_of(col_miete)) %>%
  haven::zap_labels() %>%
  haven::zap_formats() %>%
  haven::zap_label() %>%
...
data_miete %>%
  write_parquet("./data/RWI_GEO_RED/Daten_Miete/data_miete.parquet")
```
...
dates_pseudo_halbjahr %>%
  write_parquet("./data/dates_pseudo_halbjahr.parquet")
```
...
cells_kreise_einkommen <- read_parquet("./data/Geodaten/Raster_1km_kreise_einkommen.parquet")

cells_gemeinden_gebäudetypen <- read_parquet("./data/Geodaten/Raster_1km_gemeinden_gebäudetypen.parquet")

cells_baujahr <- open_dataset("./data/Zensus/Gesamt/") %>%
  filter(typ == "1km") %>%
...
cells_zensus_sonst <- open_dataset("./data/Zensus/Gesamt/") %>%
  filter(typ == "1km") %>%
...
erklärende_imp %>%
  write_parquet("./data/Sonstige/erklärende_imp.parquet")
```
...
```{r}
# Einlesen
data_kauf <- read_parquet("./data/RWI_GEO_RED/Daten_Kauf/data_kauf.parquet")
...
data_kauf_complete %>%
  write_parquet("./data/RWI_GEO_RED/Daten_Kauf/data_kauf_complete.parquet")
```
...
```{r}
# Einlesen
data_miete <- read_parquet("./data/RWI_GEO_RED/Daten_Miete/data_miete.parquet")
...
data_miete_complete %>%
  write_parquet("./data/RWI_GEO_RED/Daten_Miete/data_miete_complete.parquet")
```
