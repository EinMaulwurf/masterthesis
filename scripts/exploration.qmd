...
read_parquet("./data/processed/RWI_GEO_RED/data_miete_complete.parquet") %>%
  group_by(x_mp, y_mp) %>%
...
kreise %>%
  ...
  mutate(einkommen_cut = santoku::chop_equally(einkommen, groups = 4,
...
kreise %>%
  left_join(einkommen, by = "ars") %>%
  mutate(einkommen_delta_cut = santoku::chop_equally(einkommen_delta, groups = 4,
...
data_kauf_complete %>% 
  ...
  mutate(einkommen = einkommen / 1000,
         anzahl_haushalte = anzahl_haushalte / 1000) %>%
  filter(median_baujahr > quantile(.$median_baujahr, 0.05),
...
ggsave("./output/plots/plot_erklärende_variablen.pdf", width = 8, height = 8)
```
