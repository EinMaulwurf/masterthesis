...
for(d in arrow_dates) {
  ...
  for(v in arrow_variables) {
    ...
    breitband_100m_arrow %>%
      ...
      group_by(date, name) %>%
      write_dataset("./data/Breitbandatlas/Raster_1km_long/")
...
```{r}
breitband_1km_long_arrow <- open_dataset("./data/Breitbandatlas/Raster_1km_long/")
...
for(d in arrow_dates) {
  ...
  breitband_1km_long_arrow %>%
    ...
    group_by(date) %>%
    write_dataset("./data/Breitbandatlas/Raster_1km/")
}
```
...
breitband_1km_arrow %>%
  ...
  write_parquet("./data/Breitbandatlas/Raster_1km_max_speed.parquet")
```
...
breitband_1km_arrow %>%
  ...
  arrange(x_mp, y_mp, date) %>%
  write_parquet("./data/Breitbandatlas/Raster_1km_problems.parquet")
```
...
```{r}
breitband_problems <- read_parquet("./data/Breitbandatlas/Raster_1km_problems.parquet")
...
breitband_1km_arrow %>%
  ...
  ungroup() %>%
  write_parquet("./data/Breitbandatlas/Raster_1km_got_fiber_date.parquet")
```
