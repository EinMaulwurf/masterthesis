...
for(i in seq_along(grid_chunks)) {
  ...
  write_parquet(chunk_result, str_glue("./data/Breitbandatlas/Rohdaten/raster_chunk_{i}.parquet"))
  rm(chunk_result)
  gc()
}
plan(sequential)
...
```{r}
dataset_old <- list.files(path = "./data/Breitbandatlas/Rohdaten",
...
for(d in dates %>% head(-6) %>% unname()) {
  ...
  dataset_old %>%
    ...
    distinct() %>%
    group_by(date) %>%
    write_dataset("./data/Breitbandatlas/Raster_100m")
...
data_202312 <- open_dataset("./data/Breitbandatlas/Raster_100m/") %>%
  filter(date == 202312) %>%
  select(x_mp, y_mp) %>%
  distinct() %>%
  collect()
