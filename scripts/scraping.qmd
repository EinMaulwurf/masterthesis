...
for(i in seq_along(grid_chunks)) {
  ...
  write_parquet(chunk_result, str_glue("./data/processed/Breitbandatlas/Rohdaten/raster_chunk_{i}.parquet"))
  rm(chunk_result)
  gc()
}
plan(sequential)
...
```{r}
dataset_old <- list.files(path = "./data/processed/Breitbandatlas/Rohdaten",
