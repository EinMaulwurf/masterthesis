...
**Achtung:** Bis einschließlich 07.2021 sind die Daten auf einem 250m Raster!
...
for(d in unname(dates[1:4])) {
  ...
  dataset_old %>%
    ...
    select(date, x_mp, y_mp, starts_with("down")) %>%
    distinct() %>%
    group_by(date) %>%
...
# Jahre mit 250m Raster, das aufgeteilt werden muss
sourceCpp("get_coords.cpp")
...
```{r}
final_result <- open_dataset("./Daten/Breitbandatlas/Raster_100m/")
