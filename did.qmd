...
mod_miete <- att_gt(
  ...
  idname = "xy_mp",
  gname = "got_fiber_date_pseudo_halbjahr",
  xformla = ~ median_baujahr + median_wohnflaeche + median_zimmeranzahl + gemeindetyp_name,
  #weightsname = "n_cell",
  data = data_miete_complete %>% filter(n_cell > 10) %>% filter(!gemeindetyp_name %in% c("Großstadt")),
  anticipation = 0,
  alp = 0.05,
...
mod_miete_agg_group <- did::aggte(mod_miete, type = "group", min_e = -4, max_e = 6)
ggdid(mod_miete_agg_group)

mod_miete_agg_calendar <- did::aggte(mod_miete, type = "calendar", min_e = -4, max_e = 6)
ggdid(mod_miete_agg_calendar)
```